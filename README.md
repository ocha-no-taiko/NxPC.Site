...
2. `src/assets/` 内の画像・動画ファイルを差し替える(パスを event.json に合わせる)
...
```jsonc
{
    ...
    "theme": {
        ...
        "fonts": {
            ...
            "body": "'平成丸ゴシック Std W4', sans-serif",   // 本文フォント(段落テキスト)
            "heading": "'平成丸ゴシック Std W6', sans-serif", // 見出し・ラベル・帯・Scrollフォント
            "performerTitle": "'平成丸ゴシック Std W6', sans-serif" // 出演チーム名のフォント
        }
    },
    ...
    "performers": [
        {
            ...
            "guest": true,                             // 任意。ゲスト枠の演出(GUESTバッジ＋グリッチ枠)を付ける
            "images": [
...
- `guest` は任意。`true` にすると、画像右上に「GUEST」ラベルと、キラカード風のグリッチするフレーム＋ホログラム演出が付きます(省略または `false` で通常表示)

### デザインについて(vol.79〜)

- **フォント**: 平成丸ゴシック Std(見出し・タイトル類は W6、本文は W4)を指定していますが、
  これは有料フォントのため配信環境(Adobe Fonts 等のWebフォントキット)が別途必要です。
  未配信の環境では自動的に `Zen Maru Gothic`(Google Fonts, 無料)にフォールバックします。
  配信の目処が立ったら `theme.fonts.stylesheets` にそのキットのURLを追加してください。
- キービジュアルに合わせ、見出し・タイトル・本文は横方向に **150%** ストレッチされています
  (`scss/style.scss` の `.heading, .title, .text` ルール)。
- info・timetable・performer を囲むパネルは「Liquid Glass」風(無色の磨りガラス・背景がうっすら透ける)で、
  パネル内の文字は黒になっています。
- ファーストビューのタイトルロゴ(Lottieアニメーション)は vol.79 で廃止しました。
  代わりに背景動画自体がキービジュアルとして機能します。
